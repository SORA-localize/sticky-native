...
### migration SSOT unavailable 時の正式代替手順

`/Users/hori/Desktop/Sticky/migration/*` が作業環境に存在しない場合、未確認のまま黙って進めることは禁止する。  
ただし、以下をすべて満たす場合に限り、ローカル SSOT と現行実装を根拠に調査・計画・限定実装へ進んでよい。

必須条件:

- 計画書の `SSOT参照宣言` に migration unavailable の事実を明記する
- 変更対象を 1 レイヤまたは 1 機能帯に限定し、影響範囲を明文化する
- `問題一覧` に migration unavailable 自体を `K-` issue として載せる
- `Gate条件` に「migration SSOT を復旧して再照合する」または「本ガイドの代替手順で進む制約を再確認する」を入れる
- `回帰/副作用チェック` と `実機確認項目` を通常より厳しく書く
- 上位 SSOT が無くても進める理由と、今回まだ扱わないことを明文化する

制約:

- Home / Trash / Session / persistence schema など広い責務へまたがる変更には使わない
- 新規データモデル追加、schema 変更、window lifecycle 変更を複数同時に持ち込まない
- 高リスク領域では probe phase を本実装前に分離する

実装着手の閾値:

- 調査・計画・一時プローブはこの代替手順で進めてよい
- 本実装へ進むときは、計画書に代替手順適用中であること、変更範囲が限定されていること、未確認の上位 SSOT による残余リスクを書く
...
- migration unavailable の場合は、本ガイドの正式代替手順に従っている
...
[ ] migration unavailable の場合、正式代替手順を計画書へ反映した
